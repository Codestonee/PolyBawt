...
1. **Memory Leak in Dedup Set**: `_processed_fill_ids` set grows unbounded, only trimmed after reaching 10,000 entries (lines 710-711 in `value_betting.py`).
2. **Blocking Sync Calls in Async Context**: `event_bus.publish_sync()` is called from async functions, potentially blocking the event loop.
3. **Order Book Cache Stale Data**: 60-second TTL for order book data is too long for HFT trading; prices may be significantly stale.
4. **No Retry Logic for Market Discovery**: Unlike CLOB client, `MarketDiscovery` has no exponential backoff for API failures.
5. **Hardcoded Strategy Parameters**: Many thresholds are hardcoded in strategy classes rather than loaded from config (e.g., `min_spread = 0.05` in SpreadMaker).

### 🟡 Medium-Priority Issues

1. **Incomplete LeggedHedge State Machine**: The hedge execution uses a simplistic `1.0 - book.best_bid - 0.02` formula without market context (line 495 in `strategies.py`).
2. **No Position Limit Per Token**: Multiple strategies can open positions on the same token, exceeding intended exposure.
3. **Missing Error Context in Logs**: Many `logger.error()` calls don't include stack traces or sufficient context for debugging.
4. **VPIN Approximation**: VPIN calculation uses order book updates as volume proxies instead of actual trade flow (lines 434-441 in `value_betting.py`).
5. **No Order Size Rounding**: Polymarket likely has minimum tick sizes; orders may be rejected for precision issues.
6. **Daily Reset Only Checks UTC Midnight**: If bot runs across midnight, circuit breaker reset may not align with Polymarket's trading day.

### 🔵 Design Limitations

1. **Monolithic Ensemble Orchestrator**: `EnsembleStrategy` is 784 lines with too many responsibilities; should be split into separate services.
2. **No Backtesting Framework Integration**: `src/backtesting/engine.py` exists but isn't wired into the main strategies.
3. **Single-Threaded Async**: CPU-intensive calculations (Kelly sizing, correlation) block the event loop.
4. **No Database for Historical Data**: Only file-based persistence; no proper time-series storage for analysis.
5. **Missing Health Check Endpoint**: API server lacks `/health` or `/ready` endpoints for container orchestration.
6. **No Graceful Degradation**: If Binance WS disconnects during trading, there's no automatic strategy pause.
7. **Configuration Schema Coupling**: Config models are tightly coupled; adding new strategies requires modifying multiple config classes.
8. **No Market Session Awareness**: Doesn't account for Polymarket market lifecycle (open, trading, settlement phases).

### Testing Gaps

1. **No Integration Tests for Live Flow**: Tests are mostly unit tests; no end-to-end trading simulation.
2. **Missing Mocks for External APIs**: Tests may actually hit Binance/Polymarket APIs (network-dependent).
3. **No Load Testing**: Unknown behavior under high message rates or market volatility.
